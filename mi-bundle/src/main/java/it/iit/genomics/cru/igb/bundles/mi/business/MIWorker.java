...
public class MIWorker extends SwingWorker<ArrayList<MIResult>, String> {
    ...
    public MIWorker(List<MIResult> results, IgbService service, MIQuery query,
        ...
        this.geneManager = IGBQuickLoadGeneManager.getInstance(service,
                query.getSpecies());
    ...
    public ArrayList<MIResult> doInBackground() {
        ...
        UniprotkbUtils uniprotUtil = UniprotkbUtils.getInstance(query
                .getTaxid());
...
//            }
        ...
        logAndPublish("get protein-protein interactions");
        ...
        for (String ac : queryUniprotAcs) {
            ...
            try {
                if (false == PsicquicInitWorker.nullServer.equals(query
                        .getPsiquicServer()) && null != query
                        .getPsiquicServer()) {
                    for (Interaction interaction : PsicquicUtils.getInstance().getInteractors(
                            query.getPsiquicServer(), ac)) {
            ...
            if (null != query.getUserStructuresPath()
                                    ...
                                    switch (polymer.getType()) {
                                        case "protein":
                                            if (null != polymer.getMacromolecule()) {
                                                ...
                                                if (false == proteinAc.equals(entry.getUniprotAc()) || polymer.getChains().size() > 1) {

                                                    interactors.getOrCreateInteraction(ac, proteinAc).addType(INTERACTION_TYPE_PDB);
                                                    uniprotNeedMapping.add(ac);
                                                }
                                            }
                                            break;
                                        case "dna":
                                            ...
                                            // Merge all DNA entries, use "DNA as name rather that the desciption
                                            MISymContainer dnaSym = symManager.getByProteinAc(MoleculeEntry.TAXID_DNA);
                                            uniprotNeedMapping.add(ac);

                                            interactors.getOrCreateInteraction(ac, MoleculeEntry.TAXID_DNA).addType(INTERACTION_TYPE_PDB);
                                        ...
                                        case "rna":
                                            ...
                                            uniprotNeedMapping.add(ac);
                                            // Merge all RNA entries, use "RNA as name rather that the desciption
                                            MISymContainer rnaSym = symManager.getByProteinAc(MoleculeEntry.TAXID_RNA);

                                            interactors.getOrCreateInteraction(ac, MoleculeEntry.TAXID_RNA).addType(INTERACTION_TYPE_PDB);
                    ...
                    if (query.searchLigands()) {
                        try {
                            for (Ligand ligand : client.getLigands(subset)) {
                                ...
                                interactors.getOrCreateInteraction(ac, ligand.getChemicalName()).addType(INTERACTION_TYPE_PDB);

                                if (misym == null) {
                                    MoleculeEntry ligandEntry = new MoleculeEntry(
            ...
            if (query.searchModifications()) {
                ...
                for (ModifiedResidue modification : entry.getModifications()) {
                    ...
                    if (misym == null) {
                        ...
                        interactors.getOrCreateInteraction(ac, modification.getDescription()).addType("direct interaction (Uniprot)");
...
//                                "association");
                        ...
                        targetUniprotEntries.put(modification.getDescription(),
                                ligandEntry);
            ...
            for (String interactorUniprotAc : interactorUniprotAcs) {
                ...
                if (QueryType.EXTRA.equals(query.getQueryType())
                        || queryUniprotAcs.contains(interactorUniprotAc)) {
...
//                            interactors.get(interactorUniprotAc));
        ...
        for (MoleculeEntry entry
                : targetUniprotEntries.values()) {
        ...
        for (MISymContainer container
                : symManager.getQueryContainers()) {
            if (null != container.getEntry()) {
                targetUniprotEntries.put(container.getEntry().getUniprotAc(),
                        container.getEntry());
            }
        }
        ...
        for (MoleculeEntry uniprotEntry
                : targetUniprotEntries.values()) {
            ...
            for (String geneId : geneIds) {
                ...
                for (MIGene gene : genes) {
                    ...
                    if (null != uniprotEntry.getVarSpliceAC(gene.getID())) {
                        ...
                        BioSeq chromosome = geneManager.getSequence(gene
                                .getChromosomeName());

                        if (chromosome == null) {
                            igbLogger.severe(
                                    "Unavailable sequence: "
                                    + gene.getChromosomeName() + ", there may be a network problem.");
                            continue;
                        }
                ...
                for (String seq : merger.getSequences()) {
                    ...
                    for (Range range : merger.getRanges(seq)) {
                        SeqSpan span = new SimpleSeqSpan(range.getMin(),
                                range.getMax(), chromosome);
        ...
        for (String ac : uniprotNeedMapping) {
            ...
            for (MIGene gene : proteinContainer.getMiGenes()) {
                ...
                for (SeqSymmetry selectedSym : miGene2selectedSyms.get(gene)) {
                    ...
                    Collection<AAPosition> positions = AAPositionManager
                            .getAAPositionManager(query.getLabel())
                            .getAAPositions(
                                    gene, selectedSym.getSpan(0).getMin(), selectedSym.getSpan(0).getMax() - 1);
        ...
        progressManager.nextMajorStep(symManager.getQueryContainers()
                .size());
        ...
        for (MISymContainer container : symManager.getQueryContainers()) {
            ...
            for (String targetUniprotAc : uniprotAc2uniprotAcs
                    .get(queryUniprotAc)) {
                ...
                // avoid this.
                if (interactionsDone.contains(targetUniprotAc + "#"
                        + queryUniprotAc)
                        || interactionsDone.contains(queryUniprotAc + "#"
                                + targetUniprotAc)) {
                    continue;
                }
                interactionsDone
                        .add(queryUniprotAc + "#" + targetUniprotAc);

                MISymContainer targetContainer = symManager
                        .getByProteinAc(targetUniprotAc);
                ...
                MIResult result = new MIResult(trackId, container,
                        targetContainer, interactors.getOrCreateInteraction(container.getEntry().getUniprotAc(), targetContainer.getEntry().getUniprotAc()), query, symManager);
     ...
     * @return
     */
    private void getGenes(ArrayList<SeqSymmetry> selectedSymmetries, BioSeq seq,
            boolean ignoreIfNoProtein) {
        ...
        MapOfMap<SeqSymmetry, MIGene> candidates = geneManager.getBySymList(seq,
                selectedSymmetries);
    ...
    protected void done() {
        ...
        for (MISymContainer container : symManager.getQueryContainers()) {
            ...
            String protein = "<b>" + container.getEntry().getGeneName()
                    + "</b>/" + container.getEntry().getUniprotAc();
        ...
        String querySummary = "Query: type="
                + query.getQueryType()
                + ", "
                + PsicquicUtils.getInstance().getServerName(
                        query.getPsiquicServer());
        ...
        if (querySummaryParts.size() <= 5) {
            ...
            querySummary += querySummaryParts.size()
                    + " genes on chromosome(s) "
    ...
    public void addResultTab(String summary, List<MIResult> results, String label, MIQuery query) {
        ...
        JTabbedPane resultsTabbedPan = MIView.getInstance()
                .getResultsTabbedPan();

        MIResultPanel resultPane = new MIResultPanel(service, summary, results,
                label, query);
        ...
        ActionListener listener = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (JOptionPane.showConfirmDialog(new JFrame(),
                        "Are you sure you want to remove this tab? All results will be lost!",
                        "Remove tab", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                    tabbedPane.remove(c);
                }
            }
        };
    ...
    private void createTrack() {
        ...
        for (TierGlyph t : service.getAllTierGlyphs()) {
            ...
            if (TierGlyph.TierType.ANNOTATION.equals(t.getTierType()) && (t.getAnnotStyle().getTrackName().equals(trackId))) {
